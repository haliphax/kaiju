...
function getChunk(x, y)
{
	var rval = false;
	$.ajax({
		url: kaiju_globals.base_url + 'mapedit/chunk/' + x + '/' + y,
		type: 'GET',
		dataType: 'json',
		async: false,
...
function modcells(sub)
{
	...
	$.ajax({
		...
		dataType: 'text',
		async: false,
		url: kaiju_globals.base_url + 'mapedit/modcells',
		success: function(ret)
		{
...
$(function()
{
	...
	$('#btn_minimap').click(function()
	{
		var x = $('body').data('x');
		var y = $('body').data('y');
		$('#minimap_img').attr('src', kaiju_globals.base_url + 'mapedit/mini/' + x + '/' + y + '/'
			+ new Date().getTime());
		$('#minimap').dialog('open');
	});
	...
	$('#minimap_img').attr('src', kaiju_globals.base_url + 'mapedit/mini/1/1');
	getChunk(1, 1);	
});